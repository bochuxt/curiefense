                  ...
                  <table class="table is-narrow is-fullwidth">
                    <tbody v-for="(row, idx) in rows" :key="idx" style="border-bottom: 1px solid lightgray;">
                      <tr @click="row_entry_idx = (row_entry_idx === idx ? -1 : idx)"
                        ...
                        >
                        <td class="is-size-7 has-text-centered  " :class="statuscode_class(row.responsecode)">{{row.responsecode}}</td>
                        <td class="is-size-7" :title="row.curiefense.attrs.ip + ':' + row.downstreamremoteaddressport">{{row.curiefense.attrs.ip}}</td>
                        <td class="is-size-7"><span class="has-text-weight-medium is-family-secondary">{{row.requestmethod}}</span> <span :title="fulluri(row)">{{suburi(row)}}</span></td>

                        <td class="is-size-7 is-120-px">&#8593;{{row.requestheadersbytes + row.requestbodybytes}} &#8595;{{row.responseheadersbytes + row.responsebodybytes}}</td>


                        <td v-if="row.upstreamremoteaddress" class="is-size-7" :title="row.upstreamremoteaddress + ':' + row.upstreamremoteaddressPort">{{row.upstreamremoteaddress}}</td>
                            ...
                            <div class="tile is-parent ">
                              <article class="tile is-child box has-background-danger-light">
                                <div class="content" style="overflow-wrap: anywhere;">
                                  ...
                                  <pre class="is-size-7 is-family-code has-background-danger-light" style="width: 932px; overflow-wrap: anywhere;">{{break_reason(row.curiefense.attrs.block_reason)}}</pre>
                                </div>
                              </article>
                            </div>
                          </div>
                                  ...
                                  <article class="tile is-child box">
                                    <div class="content" style="overflow-wrap: anywhere;">
                                      <section v-if="!isempty(row.curiefense.headers)">
                                        <label class="label">Headers<span class="is-pulled-right">{{row.requestheadersbytes}} bytes</span></label>
                                      ...
                                      <section v-if="!isempty(row.curiefense.args)">
                                        <hr/>
                                        <label class="label">Arguments<span class="is-pulled-right">{{row.requestbodybytes}} bytes</span></label>
                                        <table class="table is-narrow borderless">
                                        ...
                                        <div class="content">
                                          <div class="columns">
                                            <div class="column">
                                              ...
                                              <section class="is-size-7"><span>TTFB: {{subNum10(row.timetofirstupstreamtxbyte)}}</span></section>
                                              <section class="is-size-7"><span>TTFB RX: {{subNum10(row.timetolastupstreamtxbyte)}}</span></section>
                                              <section class="is-size-7"><span>TTLB RX: {{subNum10(row.timetofirstupstreamrxbyte)}}</span></section>
                                              <section class="is-size-7"><span>TTLB TX: {{subNum10(row.timetolastupstreamrxbyte)}}</span></section>
                                              <section class="is-size-7" v-if="row.upstreamremoteaddress"><span>Remote: {{row.upstreamremoteaddress}}:{{row.upstreamremoteaddressport}}</span></section>
                                              <section class="is-size-7" v-if="row.upstreamlocaladdress"><span>Local: {{row.upstreamlocaladdress}}:{{row.upstreamlocaladdressport}}</span></section>
                                              <section class="is-size-7" v-if="row.upstreamcluster"><span>Cluster: {{row.upstreamcluster}}</span></section>
                                              <section class="is-size-7" v-if="row.upstreamtransportfailurereason"><span>Failure: {{row.upstreamtransportfailurereason}}</span></section>
                                              <section class="is-size-7" v-if="row.routename"><span>Route Name: {{row.routename}}</span></section>
                                            </div>
                                            <div class="column">
                                              <label class="has-text-weight-semibold">Downtream</label>
                                              <section class="is-size-7"><span>TTFB TX: {{subNum10(row.timetofirstdownstreamtxbyte)}}</span></section>
                                              <section class="is-size-7"><span>TTLB TX: {{subNum10(row.timetolastdownstreamtxbyte)}}</span></section>
                                              <section class="is-size-7"><span>Remote: {{row.downstreamremoteaddress}}:{{row.downstreamremoteaddressport}}</span></section>
                                              <section class="is-size-7"><span>Local: {{row.downstreamlocaladdress}}:{{row.downstreamlocaladdressport}}</span></section>
                                              <section class="is-size-7"><span>Direct: {{row.downstreamdirectremoteaddress}}:{{row.downstreamdirectremoteaddressport}}</span></section>
                                              <section class="is-size-7"><span>Status code: {{row.responsecodedetails}}</span></section>
                                            </div>
                                            <div class="column">
                                              <label class="has-text-weight-semibold">TLS Info</label>
                                              <section class="is-size-7"><span>Version: {{row.tlsversion}}</span></section>
                                              <section class="is-size-7"><span>Cipher Suite: {{row.tlsciphersuite}}</span></section>
                                              <section class="is-size-7"><span>Cert: {{row.localcertificateproperties}}</span></section>
                                              <section class="is-size-7"><span>SNI Hostname: {{row.tlssnihostname}}</span></section>
                                              <section class="is-size-7"><span :title="row.tlssessionid">Session Id: {{row.tlssessionid.substr(0,12)}}</span></section>
                                            </div>
                                          </div>
                                        </div>
                                      </section>
                            ...
                            <div class="tile is-parent">
                              <article class="tile is-child box">
                                <div class="content">
                                  ...
                                  <section class="tag2 has-background-white-bis" v-for="tag in tags(row.curiefense.attrs.tags)" :key="tag">
                                    <span>
                                  ...
                                  <section class="is-size-7 tag2 has-background-white-bis-3">AS {{row.curiefense.attrs.asn }}</section>
                                  <section class="is-size-7 tag2 has-background-white-bis-3">{{row.curiefense.attrs.company }}</section>
                                  <section class="is-size-7 tag2 has-background-white-bis-3">{{row.curiefense.attrs.country }}</section>
                                  <section class="is-size-7 tag2 has-background-white-bis-3">rowid: {{row.rowid }}</section>
...
export default {
  ...
  computed: {
    rows() {
      return this.ld.filter(this.dbrows, (row)=>{ return row.curiefense.attrs })
    }
  },
  ...
  methods: {
    ...
    buildQuery() {
      ...
      if (this.log_filter_input.trim().length > 0) {
        let dynamic_filter = this.ld.join(this.ld.map(this.log_filter_input.split(", "), (val)=> { return `json_row ~ '${val}'` }), " AND ");
    ...
    loadDatabases() {
      ...
      this.ax(axios.post, '/exec/', payload)
        .then( (response) => {
          this.loading = false;
    ...
    ax( axios_method, urlTail, data) {
      ...
      let apiroot = this.apiRoot,
          ...
          apiurl = `${apiroot}/${apiversion}${urlTail}`;
  ...
  mounted() {
    this.loading = true
    this.buildQuery()
    setTimeout(this.loadDatabases, 3000)
  }
