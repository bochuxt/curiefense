          ...
          <table class="table">
            <thead>
              <tr>
                ...
                <th class="is-size-7" colspan="2"><span>Match</span>&nbsp;<span><i class="fas fa-sort-alpha-down"></i></span></th>
            ...
            <tbody v-for="(map_entry, idx) in selectedDoc.map" :key="idx">
              <tr @click="changeSelectedMapEntry(idx)"
                ...
                >{{ waf_profile_name(map_entry.waf_profile) ? waf_profile_name(map_entry.waf_profile)[1] : '' }}</td>
                ...
                >{{ acl_profile_name(map_entry.acl_profile) ? acl_profile_name(map_entry.acl_profile)[1] : '' }}</td>
                          ...
                          <div class="column is-8">
                              ...
                              <table class="table is-hoverable is-narrow is-fullwidth">
                                <thead>
                                  ...
                                  <th class="has-text-centered is-size-7 is-60-px">
                                    <a v-if="limit_rules && map_entry.limit_ids && limit_rules.length > map_entry.limit_ids.length"
                                ...
                                <tbody>
                                  <tr v-for="(limit_id, idx) in map_entry.limit_ids" :key="limit_id" :class="{ 'highlighted': rateLimitAnalyzed ? rateLimitAnalyzed.id === limit_id : false }">
                                    <td class="is-size-7">
                                    ...
                                    <td class="is-size-7" v-if="limit_details(limit_id)">{{ limit_details(limit_id).name}}</td>
                                    <td class="is-size-7" v-if="limit_details(limit_id)">{{ limit_details(limit_id).description}}</td>
                                    <td class="is-size-7" v-if="limit_details(limit_id)">{{ limit_details(limit_id).limit}}</td>
                                    <td class="is-size-7" v-if="limit_details(limit_id)">{{ limit_details(limit_id).ttl}}</td>
                                      ...
                                      <div class="control is-expanded">
                                        <div class="select is-small is-size-7 is-fullwidth">
                                          <select class="select is-small" v-model="limit_new_entry_id">
                                            <option v-for="rule in newLimitRules(map_entry.limit_ids)" :key="rule.id" :value="rule.id">{{ rule.name + ' ' + rule.description }}</option>
                                          </select>
                                        </div>
                                      </div>
                                    </td>
                                  ...
                                  <tr v-if="map_entry.limit_ids && map_entry.limit_ids.length === 0 ">
                                    <td colspan="5">
                                      <p class="is-size-7 has-text-grey has-text-centered ">
                                        To attach an existing rule, click <a title="Add New" @click="limit_new_entry_mode_map_entry_id = map_entry.key">here</a>.
                                        <br/>
                            ...
                            <div class="content is-size-7 has-text-grey">
                              ...
                              <span v-if="rateLimitRecommendationStatus === 'recommend' && rateLimitAnalyzed">
                                Recommended rate limit threshold based on data from the last seven days is <b>{{ rateLimitRecommendation }}</b>.
                              ...
                              <span v-if="rateLimitRecommendationStatus === 'error'" class="has-text-danger has-background-danger-light">
                              ...
                              <span v-if="rateLimitRecommendationStatus === 'loading'">
                                <button class="button is-outlined is-text is-small is-loading" v-if="rateLimitRecommendationLoading">Loading</button>
                              </span>
                            </div>
                              ...
                              <div class="control is-expanded">
                                <div class="select is-fullwidth is-small">
                                  <select v-model="map_entry.waf_profile">
                                    <option v-for="waf in waf_profiles" :value="waf[0]" :key="waf[0]">{{ waf[1] }}</option>
                                  </select>
                                </div>
                              </div>
                            </div>
                              ...
                              <div class="control is-expanded">
                                <div class="select is-fullwidth is-small">
                                  <select v-model="map_entry.acl_profile">
                                    <option v-for="acl in acl_profiles" :value="acl[0]" :key="acl[0]">{{ acl[1] }}</option>
                                  </select>
                                </div>
                              </div>
                            </div>
...
export default {
  ...
  computed : {
    ...
    waf_profiles() { return this.additioanl_props[0] },
    acl_profiles() { return this.additioanl_props[1] },
    limit_rules() { return this.additioanl_props[2] }
  ...
  methods: {
    ...
    acl_profile_name(id) {
      return this.ld.find(this.acl_profiles, (profile) => {
        return profile[0] === id
      })
    },
    waf_profile_name(id) {
      return this.ld.find(this.waf_profiles, (profile) => {
        return profile[0] === id
      })
    },

    newLimitRules(limit_ids) {
      let new_ids = this.ld.filter(this.limit_rules, (rule) => {
    ...
    limit_details(limit_id) {
      return this.ld.find(this.limit_rules, (rule)=> { return rule.id == limit_id})
    },
    ...
    addNewProfile(map, idx) {
      ...
      element.focus();
      setTimeout(()=>{element.setSelectionRange(0, 148)}, 100)
    },
    ...
    calcRateLimitRecommendation(mapEntry, rateLimit) {
      ...
      DatasetsUtils.Analyze('rate-limit-recommendation',
          {
    ...
    applyRateLimitRecommendation() {
      ...
      if (this.isRateLimitReferencedElsewhere(this.rateLimitAnalyzed.id, this.mapEntryAnalyzed)) {
        ...
        this.$emit('updateDoc', axios.post, recommendedRateLimit, 'limits', recommendedRateLimit.id)
        // This timeout allows enough time to pass for the new rate limit rule to be saved before we attempt to access its data in the map entries table.
        // TODO: After rebuilding the requests system to work from a single source, we will be able to update the list after the requests sends an 'ok' response
        setTimeout( () => {
        ...
        this.$emit('updateDoc', axios.put, recommendedRateLimit, 'limits', recommendedRateLimit.id)
        this.clearRateLimitRecommendation()
      }
    },
  ...
  mounted() {
    console.log("this.additioanl_props", this.additioanl_props)
  }
}
</script>
